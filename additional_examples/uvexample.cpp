...
int main() {
    config_parser configure("additional_examples/uvexample.yaml");
    std::cout << "U polynomial = " << configure.U() << std::endl;
    std::cout << "F polynomial = " << configure.F() << std::endl;
    std::cout << "To compute master integrals at higher dimension, left multiply by" << std::endl;
    std::cout << ((GiNaC::ex)configure.get_shift_to_lower_dim()).normal() << std::endl;
    std::cout << "where d represents the higher dimension" << std::endl;
    std::cout << "Notice that we still want our master integrals have all non-negative indices" << std::endl;
